...
class CharacteristicOutput(BaseModel):
    ...
    __properties = ["id", "name", "label", "description", "text", "dictionaryId", "notes", "instanceType"]

    @validator('instance_type')
    def instance_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Characteristic',):
            raise ValueError("must be one of enum values ('Characteristic')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CharacteristicOutput:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CharacteristicOutput:
        """Create an instance of CharacteristicOutput from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CharacteristicOutput.parse_obj(obj)

        _obj = CharacteristicOutput.parse_obj({
            "id": obj.get("id"),
            "name": obj.get("name"),
            "label": obj.get("label"),
