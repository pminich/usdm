...
class ProcedureOutput(BaseModel):
    ...
    code: Code = Field(...)
    study_intervention_id: Optional[StrictStr] = Field(default=None, alias="studyInterventionId")
    notes: Optional[conlist(CommentAnnotation)] = None
    instance_type: StrictStr = Field(default=..., alias="instanceType")
    __properties = ["id", "name", "label", "description", "procedureType", "code", "studyInterventionId", "notes", "instanceType"]

    @validator('instance_type')
    def instance_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Procedure',):
            raise ValueError("must be one of enum values ('Procedure')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ProcedureOutput:
    ...
    def to_dict(self):
        ...
        # set to None if study_intervention_id (nullable) is None
        # and __fields_set__ contains the field
        if self.study_intervention_id is None and "study_intervention_id" in self.__fields_set__:
            _dict['studyInterventionId'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> ProcedureOutput:
        """Create an instance of ProcedureOutput from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ProcedureOutput.parse_obj(obj)

        _obj = ProcedureOutput.parse_obj({
            "id": obj.get("id"),
            "name": obj.get("name"),
            "label": obj.get("label"),
            ...
            "study_intervention_id": obj.get("studyInterventionId"),
